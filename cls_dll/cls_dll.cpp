// cls_dll.cpp : Defines the exported functions for the DLL application.
//
...
// see classify.h for the class definition
Ccls::Ccls(std::string img_path)
{
	Session* session;
	Status status = NewSession(SessionOptions(), &session);//创建新会话Session
	string model_path = "./model/classify.pb";
	...
	std::vector<std::pair<string, tensorflow::Tensor>> inputs = {
		{ "Placeholder", resized_tensor },
	};
	string output_node = "logits";
